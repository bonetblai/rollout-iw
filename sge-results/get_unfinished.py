...
# obtain filenames of unfinished experiments downpath current folder, the
# files to be processed are those whose names begin with given prefix and
...
if len(sys.argv) != 3:
    ...
    print "Prints filenames of unfinished experiments downpath of current folder."
...
for root, dirs, files in os.walk("."):
    for filename in files:
        if filename[:len(filename_prefix)] == filename_prefix and filename[-len(filename_suffix):] == filename_suffix:
            ...
            if "episode-stats" not in last_line:
                print filename
